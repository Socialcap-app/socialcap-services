...
export { TransactionsQueue, postTransaction, WAITING, REVISION, DONE, FAILED, IGNORED, MAX_RETRIES };
class TransactionsQueue {
   ...
   */
     ...
     */
    async getFirstWaitingTransaction(types) {
    ...
    /**
     */
    async updateTransaction(uid, params) {
     ...
     */
    async retryTransaction(uid, MAX_RETRIES) {
     ...
     * @returns the updated Transaction
     */
    async closeTransaction(uid, params) {
        let { state, result } = params;
 ...
 * to create a Queue.
 */
async function postTransaction(queueId, params) {
    let tx = await TransactionsQueue
        .queue(queueId)
        .push(params);
    log.postedTxn(tx);
}
//# sourceMappingURL=transaction-queues.js.map