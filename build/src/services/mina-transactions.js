...
async function waitForTransaction(txnId, params, onSuccess, onError) {
    ...
    let timer = setTimeout(async () => {
        ...
        if (done && done.failureReason !== null) {
            let message = (done.failureReason || []).map((t) => {
                return t.failures[0];
            }).join(", ");
 ...
 * @returns
 */
async function queryTxnStatus(txnId) {
    try {
...
async function onlyIfMain() {
    ...
    if (!argvs[1].includes("mina-transactions.js"))
        return;
    /*
    ...
    */
    waitForTransaction("5JvKrrANa1Vtmpd7XqmHVQu8d8EkjEGesFVsetrMrmDTbTWB0000", {}, (params) => {
        console.log("DONE");
    }, (err) => {
        console.log("ERR", err);
    });
    waitForTransaction("5JvKrrANa1Vtmpd7XqmHVQu8d8EkjEGesFVsetrMrmDTbTWB0001", {}, (params) => {
        console.log("DONE");
    }, (err) => {
        console.log("ERR", err);
    });
    waitForTransaction("5JvKrrANa1Vtmpd7XqmHVQu8d8EkjEGesFVsetrMrmDTbTWB0002", {}, (params) => {
        console.log("DONE");
    }, (err) => {
        console.log("ERR", err);
    });
...
onlyIfMain();
//# sourceMappingURL=mina-transactions.js.map