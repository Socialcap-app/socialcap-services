...
import { CLAIMED, WAITING, UNPAID } from "@socialcap/contracts";
...
export async function getMyClaimables(params) {
    const userUid = params.user.uid;
    // all commnunity Uids where is a a member
...
export async function submitClaim(params) {
    ...
    if (waitForPayment) {
        ...
        rs = await updateEntity("claim", uid, claim);
        waitForTransaction(transaction.hash, params, async (params) => {
