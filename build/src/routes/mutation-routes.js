...
async function mutationRoutes(fastify, options) {
    fastify.post('/api/mutation/:method', async (request, reply) => {
        ...
        if (!handler) {
            return hasError.MethodNotSupported(_.method_not_supported(method));
        }
        ...
        try {
            ...
            reply.code(UNKNOWN_ERROR).send(hasError.Unknown(_.unknown_error(method, `params=${JSON.stringify(params)} error=${err}`)));
        }
    });
}
export default mutationRoutes;
//# sourceMappingURL=mutation-routes.js.map